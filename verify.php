...
if (isset($_POST['verify'])) {
  ...
  $email_sql   = "SELECT * FROM users WHERE email='$email' AND vkey='$vkey'";
  $email_query = mysqli_query($connect,$email_sql);
  $num_count   = mysqli_num_rows($email_query);
  if ($num_count != 0) {
    $user_row  = mysqli_fetch_assoc($email_query);
    $user_vkey = $user_row['vkey'];
    $user_id   = $user_row['id'];

    $user_update = "UPDATE users SET vkey='$user_vkey', verified='1' WHERE id='$user_id'";
    $update_query = mysqli_query($connect,$user_update);

    if ($update_query) {
      $_SESSION['success'] = "Your email registration for medu account verified has been successfully.Now you can login";
    ...
    $_SESSION['error'] = "Your E-mail or Varification code is wrong.please try again!!"; 
       ...
       <div class="col-md-6 m-auto">
         <div class="">
           <div class="card-header bg-info text-white text-center clearfix">
            <h5 class="text-center">Veriry Your Email for medu account</h5>
               ...
               <form action="" method="post">
                  ...
                  <div class="form-group">
                    ...
                    <div class="input-group">
                      ...
                      <input type="password" name="vkey" id="password" placeholder="Verification code" class="form-control" value="<?php if(isset($_POST['vkey'])){ echo $_POST['vkey']; } ?>">
